...
class HtmlToHwpx {
  ...
  _handleShape(divContent, paraPrId = null) {
    ...
    const styleEntry = kvPairs.find(([k]) => k === 'style');
    const styleStr = styleEntry ? styleEntry[1] : '';
    const shape = this._parseShapeStyle(styleStr);
  ...
  _processBlocksForTableCell(blocks) {
    ...
    for (const block of blocks) {
      ...
      if      (bt === 'Para')          result.push(this._handleParaInTable(bc, block.paraStyle));
        ...
        if (classes.includes('hwpx-shape')) result.push(this._handleShape(bc));
    ...
    for (const item of items) {
      ...
      for (const block of item) {
        ...
        if (bt === 'Plain' || bt === 'Para') {
          ...
          const classes = bc && bc[0] && bc[0][1] ? bc[0][1] : [];
          if (classes.includes('hwpx-shape')) result.push(this._handleShape(bc, paraPrId));
  ...
  _handleBlockQuote(content, paraPrId = null) {
    ...
    for (const block of (content || [])) {
      ...
      if (bt === 'Para' || bt === 'Plain')
        ...
        const classes = bc && bc[0] && bc[0][1] ? bc[0][1] : [];
        if (classes.includes('hwpx-shape')) result.push(this._handleShape(bc, bqId));

...
class HtmlToHwpx {
    ...
    for (const part of styleStr.split(';')) {
      ...
      switch (key) {
        case 'text-align': {
          ...
          else if (a === 'justify') result.align = 'BOTH';
          break;
        }
        case 'padding-left':
        case 'margin-left': {
  ...
  _getOrCreateParaPr(paddingLeft = 0, textIndent = 0, align = null, marginRight = 0, spaceBefore = 0, spaceAfter = 0) {
    ...
    const cacheKey    = `${leftMargin},${indentVal},${alignKey},${rightVal},${prevVal},${nextVal}`;
    if (this.paraPrCache.has(cacheKey)) return this.paraPrCache.get(cacheKey);
    if (leftMargin === 0 && indentVal === 0 && !align && rightVal === 0 && prevVal === 0 && nextVal === 0) return String(this.normalParaPrId);
  ...
  _handleTable(content, paraPrId = null) {
    ...
    for (let rowIdx = 0; rowIdx < rowCnt; rowIdx++) {
      ...
      for (let colIdx = 0; colIdx < colCnt; colIdx++) {
        ...
        // Resolve a borderFill that matches this cell's CSS border styles
        const cellBorders  = this._parseCellBorderStyle(attr);
        const cellBfId     = this._getOrCreateCellBorderFill(cellBorders);
   ...
   */
  _getOrCreateCellBorderFill(borders) {
    if (this.tableBorderFillId === null) this._ensureTableBorderFill();
    ...
    // If all four sides match the table default, reuse the shared fill
    if (isDefault(left) && isDefault(right) && isDefault(top) && isDefault(bottom)) {
      return this.tableBorderFillId;
    }
    ...
    const cacheKey =
      ...
      `${bottom.type}:${bottom.width}:${bottom.color}`;
  ...
  _handleParaInTable(content, paraStyleStr = null) {
    ...
    if (paraStyleStr) {
      ...
      if (ps.align || ps.paddingLeft || ps.textIndent || ps.marginRight || ps.spaceBefore || ps.spaceAfter) {
        paraPrId = this._getOrCreateParaPr(
          ps.paddingLeft || 0, ps.textIndent || 0, ps.align || null,
          ps.marginRight || 0, ps.spaceBefore || 0, ps.spaceAfter || 0
        );
      }
    }
  ...
  _handlePara(content, paraStyles = null, paraStyleStr = null) {
    let paddingLeft = 0, textIndent = 0, align = null, marginRight = 0, spaceBefore = 0, spaceAfter = 0;
    ...
    const paraPrId = this._getOrCreateParaPr(paddingLeft, textIndent, align, marginRight, spaceBefore, spaceAfter);
    return (
      this._createParaStart(this.normalStyleId, paraPrId) +

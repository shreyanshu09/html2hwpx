...
function nodeAttr(node) {
  ...
  if (a.width)  kv.push(['width',  String(a.width)]);
  if (a.height) kv.push(['height', String(a.height)]);
...
function parseTable(tableNode) {
  ...
  if (!hasWidths) {
    ...
    if (colHints.some(h => h !== null)) {
      // Fill gaps: prefer using the table's own explicit width to compute remaining space.
      // Fallback: average of known widths.
      const known     = colHints.filter(h => h !== null);
      const knownSum  = known.reduce((a, b) => a + b, 0);
      const nullCount = colHints.filter(h => h === null).length;
      const tableW    = widthToRelative(extractNodeWidth(tableNode));
      let fill;
      if (tableW !== null && nullCount > 0) {
        const remaining = tableW - knownSum;
        fill = remaining > 0 ? remaining / nullCount : knownSum / known.length;
      } else {
        fill = nullCount > 0 ? knownSum / known.length : 0;
      }
      rawWidths = colHints.map(h => h !== null ? h : fill);
      hasWidths = true;
    }
  }
